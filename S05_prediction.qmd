...
covars_rcp45_2075 = read_brickman(db |> filter(scenario == "RCP45", year == 2075, interval == "mon")) |>
  select(all_of(cfg$keep_vars)) |>
  slice("month", "Aug") 
forecast_rcp45_2075 = predict_stars(wflow, covars_rcp45_2075)
forecast_rcp45_2075

covars_rcp45_2055 = read_brickman(db |> filter(scenario == "RCP45", year == 2055, interval == "mon")) |>
  select(all_of(cfg$keep_vars)) |>
  slice("month", "Aug") 
forecast_rcp45_2055 = predict_stars(wflow, covars_rcp45_2055)
forecast_rcp45_2055

rcp45 = c(nowcast, forecast_rcp45_2055, forecast_rcp45_2075, along = list(year = c("2020", "2055", "2075")))

plot_coast = function(){
  plot(coast, col = "orange", lwd = 2, add = TRUE)
}

plot(rcp45['.pred_presence'], 
     hook = plot_coast,
     axes = TRUE, breaks = seq(0, 1, by = 0.1), join_zlim  = TRUE, reset = FALSE)
...
# write individual arrays?
write_prediction(nowcast, file = file.path(path,"g_Aug_2020.tif"))
write_prediction(forecast_rcp45_2055, file = file.path(path, "g_Aug_RCP45_2055.tif"))
write_prediction(forecast_rcp45_2075, file = file.path(path, "g_Aug_RCP45_2075.tif"))

# or write them together in a "multi-layer" file?
write_prediction(rcp45, file = file.path(path, "g_Aug_RCP45_all.tif"))
```