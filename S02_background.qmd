...
for (this_month in month.abb){
  result = make_model_input_by_month(this_month,
                                     obs = read_observations(scientificname = "Placopecten magellanicus"),
                                     raster = mask,
                                     species = "Placopecten magellanicus",
                                     path = data_path("model_input"),
                                     min_obs = 3)
...
Notes on hw:
- gonna have to replicate filename = sprintf("%s-%s-greedy_input.gpkg", 
                     gsub(" ", "_", species),
                     mon)
- gonna need a replication of read_sf(greedy_input, file.path(path, filename))
- goal is to have the function read the monthly model input data files and save certain variables of the files as a table
